...
public abstract class Instrumentor
{
      ...
      for (int i = 0; i < interfaces.length; i++)
      {
         ...
         while (entries.hasNext())
         {
            ...
            if (baseMethod != null && !addedMethods.contains(hash))
            {
               ...
               {
                  if (AspectManager.verbose)logger.debug("[warn] " + msg);
               }
            }
           ...
           throws Exception
   {
      ...
      while (it.hasNext())
      {
         ...
         if (AspectManager.verbose) logger.debug("**** " + introduction.getOriginalAnnotationExpr() + " invisible: " + introduction.isInvisible() + " expr: " + introduction.getOriginalExpression());
         if (introduction.matches(advisor, clazz))
         {
            if (AspectManager.verbose) logger.debug(introduction.getAnnotation() + " binds to " + clazz.getName());
   ...
   protected boolean convertReferences(CtClass clazz) throws Exception
   {
      ...
      try
      {
         ...
         for (ReferenceClassIterator it = new ReferenceClassIterator(clazz.getRefClasses()) ; it.hasNext() ; )
         {
            ...
            catch (NotFoundException e)
            {
               ...
               {
                  logger.debug("[warn] Could not find class " + ref + " that " + clazz.getName() + " references.  It may not be in your classpath and you may not be getting field and constructor weaving for this class.");
                  if (AspectManager.verbose) logger.debug( e.getCause());
                  continue;
               }
               else
               {
      ...
      {	  
    	  if (ref != null)
    	  {
    		  ...
    		  {
    			  logger.debug("Failed to aspectize class " + clazz.getName() + ".  Could not find class it references " + ref + "  It may not be in your classpath and you may not be getting field and constructor weaving for this class.");
    		  }
    		  else
                            ...
                            ClassAdvisor advisor)
   {
      ...
      try
      {
         ...
         if (AspectManager.verbose) logger.debug("[trying to transform] " + clazz.getName());
         ...
         if (AspectManager.verbose) logger.debug("[debug] was " + clazz.getName() + " converted: " + (basicsSet || converted));
      ...
      catch (Throwable e)
      {
         if (AspectManager.suppressTransformationErrors)
         {
      	    logger.debug("AOP Instrumentor failed to transform " + clazz.getName()); 
            return false;
         }
         else
         {
   ...
   public synchronized void interceptorChainsUpdated(Collection joinpointUpdates, HotSwapper hotSwapper) {
      ...
      try {
         ...
         hotSwapper.hotSwap();
    	  ...
    	  if (AspectManager.suppressTransformationErrors)
    	  {        	 
    		  logger.debug("AOP Instrumentor failed to updated wrapping status." + e.getCause());
    	  }
    	  else
    	  {
    		  if (e instanceof TransformationException)
