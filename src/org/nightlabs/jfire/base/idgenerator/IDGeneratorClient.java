	...
	protected long[] _nextIDs(String namespace, int quantity)
	{
		try {
			...
			synchronized (namespace2cachedIDsMutex) {
				String currentOrganisationID = Login.getLogin().getOrganisationID();
			...
			synchronized (cachedIDs) {
				if (quantity > cachedIDs.size()) {
					IDGeneratorHelper idGeneratorHelper = IDGeneratorHelperUtil.getHome(Login.getLogin().getInitialContextProperties()).create();
