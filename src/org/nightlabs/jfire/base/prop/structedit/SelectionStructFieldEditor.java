...
import org.nightlabs.jfire.prop.StructFieldValue;
import org.nightlabs.jfire.prop.i18n.StructFieldValueName;
import org.nightlabs.language.LanguageCf;
import org.nightlabs.math.Base36Coder;

public class SelectionStructFieldEditor extends AbstractStructFieldEditor 
{	
	...
	public SelectionStructFieldEditor()
	{
	}
	...
	public Composite createComposite(Composite parent, int style, LanguageChooser langChooser, StructEditor structEditor)
	{
		...
		comp = new SelectionStructFieldEditComposite(parent, style, this, langChooser);		
		return comp;
	}
	...
	{
		if (field instanceof SelectionStructField)
		{
			selectionField = (SelectionStructField) field;
...
class SelectionStructFieldEditComposite extends XComposite implements LanguageChangeListener
{
	...
	public SelectionStructFieldEditComposite(Composite parent, int style, SelectionStructFieldEditor editor, LanguageChooser langChooser)
	{		
		...
		valueLabel = new Label(wrapper, SWT.NONE);
		valueLabel.setText("Possible values:");
		valueList = new ListComposite<StructFieldValueName>(wrapper, SWT.SINGLE);
		valueList.setLabelProvider(new MyLabelProvider());
		valueList.setLayoutData(new GridData(GridData.FILL_BOTH));
		XComposite.setLayoutDataMode(LayoutDataMode.GRID_DATA, valueList);
	...
	/**
	 * Updates the contents of the text box <code>valueText</code> with the currently 
	 * in the <code>valueList</code> selected {@link StructFieldValueName}.
	 */
	...
	/**
	 * Updates the text of the currently selected item in the <code>valueList</code> with the text in 
	 * the text box <code>valueText</code>.
	 */
	...
	/**
	 * Removes the currently selected value from the list and also from the structure.
	 */
	 ...
	 * @param field The {@link SelectionStructField} to be displayed.
	 */
