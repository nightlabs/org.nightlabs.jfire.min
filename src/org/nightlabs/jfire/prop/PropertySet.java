...
public class PropertySet implements Serializable, StoreCallback, AttachCallback, DetachCallback, Inheritable
{
	 ...
	 *         build to represent the persistent data, Therefore inflation is
...
//						displayName += ((II18nTextDataField) field).getText(new Locale(languageID)) + displayNamePart.getStructFieldSuffix();
	 ...
	 */
	@Persistent(persistenceModifier=PersistenceModifier.PERSISTENT)
	private boolean autoGenerateDisplayName = false;
	 ...
	 * @throws StructureViolationException if at least one {@link DataField} doesn not have its
	 ...
	 * <ol>
	 * <li>Validate if <code>propertySet</code>'s structure corresponds to this instance.</li>
	 ...
	 * was inflated according to its sturcture.
	 *
	 ...
	 * <p>
	 * This method should be overridden by sublcasses and an instance
	 ...
	 */
	private Map<StructFieldID, List<DataField>> getDataFieldsMap() {
	...
	public FieldInheriter getFieldInheriter(String fieldName) {
		// TODO this should do sth. meaningful!
		return null;
	}
