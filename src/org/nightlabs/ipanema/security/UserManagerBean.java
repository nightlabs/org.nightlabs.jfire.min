...
import org.nightlabs.ipanema.person.id.PersonID;
...
import org.nightlabs.ipanema.security.SecurityException;
import org.nightlabs.ipanema.security.User;
import org.nightlabs.ipanema.security.UserGroup;
import org.nightlabs.ipanema.security.UserLocal;
import org.nightlabs.ipanema.security.UserNotFoundException;
import org.nightlabs.ipanema.security.UserRef;
import org.nightlabs.ipanema.security.UserSearchResult;
import org.nightlabs.ipanema.security.id.AuthorityID;
import org.nightlabs.ipanema.security.id.RoleGroupID;
...
/**
 * @ejb.bean name="ipanema/ejb/JFireBaseBean/UserManager"
 *	jndi-name="ipanema/ejb/JFireBaseBean/UserManager"
 *	type="Stateless" 
 * 
...
//          "SELECT FROM org.nightlabs.ipanema.security.User " +
//          "WHERE " +
...
//          "IMPORTS import org.nightlabs.ipanema.security.UserGroup; import java.lang.String");
...
//      return (Collection)pm.detachCopyAll(c);
      ...
      Query query = pm.newQuery("SELECT FROM org.nightlabs.ipanema.security.User " +
          "WHERE " +
...
//    	return (Collection)pm.detachCopyAll(c);
  	...
  	throws ModuleException
  {
    ...
    try 
    {
      ...
      Extent ext = pm.getExtent(RoleGroup.class, false);
    	...
    	Query query = pm.newQuery( 
    			"SELECT FROM org.nightlabs.ipanema.security.RoleGroup " +
					"WHERE " +
					...
					"IMPORTS import org.nightlabs.ipanema.security.RoleGroupRef; import org.nightlabs.ipanema.security.UserRef; import org.nightlabs.ipanema.security.User; import java.lang.String");
    	...
    	query = pm.newQuery( 
    			"SELECT FROM org.nightlabs.ipanema.security.RoleGroup " +
					...
					"PARAMETERS String paramOrganisationID, String paramUserID, String paramAuthorityID " +
					"IMPORTS import org.nightlabs.ipanema.security.RoleGroupRef; import org.nightlabs.ipanema.security.UserRef; import java.lang.String");
    ...
    throws ModuleException
  {
    ...
    try 
    {
      ...
      Query query = pm.newQuery(
          "SELECT FROM org.nightlabs.ipanema.security.UserGroup " +
          "WHERE " +
...
//    Collection excludedGroups = (Collection)query.execute(userID);
      ...
      // workaround start
      query = pm.newQuery("SELECT FROM org.nightlabs.ipanema.security.UserGroup");
...
//    Collection excludedRoleGroups = (Collection)query.execute(getOrganisationID(), userID, authorityID);
      ...
      // workaround start
      query = pm.newQuery("SELECT FROM org.nightlabs.ipanema.security.RoleGroup");
...
//      return pm.detachCopyAll(res);
      ...
      // workaround start
      Query query = pm.newQuery("SELECT FROM org.nightlabs.ipanema.security.RoleGroup");
      Collection c = (Collection)query.execute();
    	query = pm.newQuery( 
    			"SELECT FROM org.nightlabs.ipanema.security.RoleGroup " +
					"WHERE " +
...
//      Collection c = (Collection)query.execute(userID);
      ...
      // workaround start
      Query query = pm.newQuery("SELECT FROM org.nightlabs.ipanema.security.UserGroup");
      Collection c = (Collection)query.execute();
    	query = pm.newQuery(
    			"SELECT FROM org.nightlabs.ipanema.security.UserGroup " +
					"WHERE " +
