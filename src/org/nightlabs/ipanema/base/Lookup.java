...
import org.nightlabs.ipanema.base.InitException;
import org.nightlabs.ipanema.jdo.cache.CacheManager;
...
import org.nightlabs.ipanema.organisation.id.OrganisationID;
 ...
 * @author marco
 */
///*// this is now ignored - we expect the system persistence manager to be always "java:/ipanema/persistenceManagers/system" 
// * @ejb.env-entry name = "ipanema/persistenceManagers/system"
// * 	type = "java.lang.String"
// * 	value = "java:/ipanema/persistenceManagers/system"
// **/
public class Lookup 
{
	...
	private JFirePrincipal ipanemaPrincipal = null;
	protected void setJFirePrincipal(JFirePrincipal _ipanemaPrincipal) {
		if (this.ipanemaPrincipal != null)
			throw new IllegalStateException("Why the hell does this instance of Lookup has already an JFirePrincipal (\""+this.ipanemaPrincipal+"\") set?");
		this.ipanemaPrincipal = _ipanemaPrincipal;
	}

	private JFireServerManagerFactory ipanemaServerManagerFactory = null;
	public JFireServerManagerFactory getJFireServerManagerFactory()
		throws ModuleException
	{
		if (ipanemaServerManagerFactory == null)
			try {
				InitialContext ctx = new InitialContext();
				try {
					ipanemaServerManagerFactory = (JFireServerManagerFactory)ctx.lookup(JFireServerManagerFactory.JNDI_NAME);
				} finally {
					ctx.close();
				}
		...
		return ipanemaServerManagerFactory;
	}

	public JFireServerManager getJFireServerManager()
	throws ModuleException
	{
		return getJFireServerManagerFactory().getJFireServerManager(ipanemaPrincipal);
	}
...
//			
////			this.ipanemaServerManagerFactory = (JFireServerManagerFactory)
...
//	}
	...
	public static Hashtable getInitialContextProps(PersistenceManager pm, String _organisationID) throws ModuleException
	{
		try {
			...
			try {
				JFireServerManagerFactory ipanemaServerManagerFactory = (JFireServerManagerFactory)
				...
				String initialContextFactory = ipanemaServerManagerFactory.getInitialContextFactory(server.getJ2eeServerType(), true);
