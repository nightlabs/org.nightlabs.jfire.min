...
//		LOGGER.info("Not yet logged in. Trying to login "+username);
		...
		try
		{
			...
			// and delegate the login to the ipanemaServerManager
			JFireServerManager ipanemaServerManager = lookup.getJFireServerManager();
			try {
				this.ip = ipanemaServerManager.login(organisationID, userID, sessionID, password);
