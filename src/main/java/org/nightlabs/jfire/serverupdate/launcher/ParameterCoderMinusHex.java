...
/**
 * This is a copy of the class <code>org.nightlabs.util.ParameterCoderMinusHex</code> because
		...
		try {
			...
			for (int index = 0; index < encoded.length(); ++index) {
				...
				if (c == ESCAPE_CHAR) {
					...
					bout.write(c); // this should be casted to int, because the bout.write(int) method must be called. Even though, there is currently no overloaded write method taking a char arg, this might change with future Java versions.
				}
			}
			bout.close();
