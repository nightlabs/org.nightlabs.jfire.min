...
public class PersonStruct
{
	...
	public static IStruct getPersonStructLocal(final PersistenceManager pm)
	{
		final String devOrganisationID = Organisation.DEV_ORGANISATION_ID;
	...
	public static void createStandardStructure(final IStruct ps, final PersistenceManager pm) {
		try {
			// *** Block Personal Data ***
			...
			final SelectionStructField salutationField = createSelectionStructField(psb, PERSONALDATA_SALUTATION,
			...
			final DateStructField dateStructField = createDateStructField(psb, PERSONALDATA_DATEOFBIRTH);
			dateStructField.setDateTimeEditFlags(IDateFormatter.FLAGS_DATE_SHORT);
			...
			final RegexStructField regexStructField = createRegexStructField(psb, INTERNET_EMAIL);
			// old regex, changed by marc 2008-04-30. Source: http://www.regular-expressions.info/email.html
			//regexStructField.setRegex("^([a-zA-Z0-9_\\-\\.])+@((([a-zA-Z0-9\\-])+\\.)+([a-zA-Z]){2,4})$");
			regexStructField.setRegex("^[a-zA-Z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-zA-Z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?\\.)+[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?$"); //$NON-NLS-1$
			...
			final SelectionStructField selField = createSelectionStructField(psb, CREDITCARD_EXPIRYMONTH,
	...
	private static void readNameFromProperties(final StructField<?> structField)
	{
	...
	private static void readNameFromProperties(final StructBlock structBlock)
	{
	...
	private static void readNameFromProperties(final MultiSelectionStructFieldValue multiSelectionStructFieldValue)
	{
		final String pk =
	...
	private static void readNameFromProperties(final StructFieldValue structFieldValue)
	{
		final String pk =
	...
	private static TextStructField createTextStructField(final StructBlock structBlock, final StructFieldID structFieldID)
	{
		TextStructField field;
		try {
			field = (TextStructField) structBlock.getStructField(structFieldID);
		} catch (final StructFieldNotFoundException x) {
			field = new TextStructField(structBlock, structFieldID);
			try {
				structBlock.addStructField(field);
			} catch (final DuplicateKeyException e) { // Should never happen, because we check for the existence of the field, first.
				throw new RuntimeException(e);
			}
		}
	...
	private static TextStructField createTextStructField(final StructBlock structBlock, final StructFieldID structFieldID, final int lineCount)
	{
		TextStructField field;
		try {
			field = (TextStructField) structBlock.getStructField(structFieldID);
	...
	private static PhoneNumberStructField createPhoneNumberStructField(final StructBlock structBlock, final StructFieldID structFieldID)
	{
	...
	private static SelectionStructField createSelectionStructField(
			final StructBlock structBlock, final StructFieldID structFieldID,
			final String ... structFieldValueIDs
	)
	{
		...
		final Map<String, StructFieldValue> structFieldValueID2value = new HashMap<String, StructFieldValue>();
		for (final StructFieldValue value : field.getStructFieldValues())
			structFieldValueID2value.put(value.getStructFieldValueID(), value);

		for (final String structFieldValueID : structFieldValueIDs) {
	...
	)
	{
		...
		final Map<String, MultiSelectionStructFieldValue> structFieldValueID2value = new HashMap<String, MultiSelectionStructFieldValue>();
		for (final MultiSelectionStructFieldValue value : field.getStructFieldValues())
			structFieldValueID2value.put(value.getStructFieldValueID(), value);
	...
	private static ImageStructField createImageStructField(
			final StructBlock structBlock, final StructFieldID structFieldID,
			final int maxSizeKB, final String ... imageFormats
	)
	{
		...
		if (field.getImageFormats().size() != imageFormats.length)
			...
			final Set<String> imageFormatsSet = CollectionUtil.array2HashSet(imageFormats);
			for (final String extension : field.getImageFormats()) {
	...
	private static DateStructField createDateStructField(final StructBlock structBlock, final StructFieldID structFieldID) {
	...
	private static StructBlock createStructBlock(final IStruct struct, final StructBlockID structBlockID) {
		StructBlock block;
		try {
			block = struct.getStructBlock(structBlockID);
		} catch (final StructBlockNotFoundException x) {
	...
	private static RegexStructField createRegexStructField(final StructBlock structBlock, final StructFieldID structFieldID) {
	...
	private static NumberStructField createNumberStructField(final StructBlock structBlock, final StructFieldID structFieldID) {
